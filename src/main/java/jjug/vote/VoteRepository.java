...
public interface VoteRepository extends Repository<Vote, UUID> {
	...
	@RestResource(exported = false)
	@Query("SELECT s.submissionId AS submissionId, s.title AS title, sp.name AS name, sp.github AS github, s.category AS category, s.level AS level, s.talkType AS talkType, count(v) AS count FROM Vote v RIGHT OUTER JOIN v.submission s JOIN s.speaker sp WHERE s.conference.confId = :confId AND s.submissionStatus=jjug.submission.enums.SubmissionStatus.SUBMITTED GROUP BY s.submissionId ORDER BY count(v) DESC, s.createdAt ASC")
